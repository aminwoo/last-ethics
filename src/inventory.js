  ...
  if (inventory.isOpen) {
    ...
    if (inventory.selectedItemIndex !== -1) {
      ...
      const selectedItem = inventory.items.find(
        (i) => i.slot === inventory.selectedItemIndex,
      )
      if (selectedItem) {
...
function showItemModel(inventory, item) {
  ...
  if (!inventory.previewRenderer) {
    ...
    renderer.setSize(
      inventory.itemPreview.clientWidth,
      inventory.itemPreview.clientHeight,
    )
    ...
    inventory.previewRenderer.setSize(
      inventory.itemPreview.clientWidth,
      inventory.itemPreview.clientHeight,
    )
  ...
  if (item.modelPath) {
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        const scale =
          (adjustment.targetLength / modelLength) * adjustment.targetScale
        ...
        if (item.id === 'pistol' || item.id === 'drakefire_pistol') {
          inventory.itemDescription.textContent =
            'Ornate handgun. High accuracy, medium damage.'
        } else if (item.id === 'sniper') {
          inventory.itemDescription.textContent =
            'Long-range precision rifle. Maximum damage at distance.'
        } else if (item.id === 'shotgun') {
          inventory.itemDescription.textContent =
            'Close-range spread weapon. Devastating against groups.'
        } else if (item.id === 'ramen') {
          inventory.itemDescription.textContent =
            'Delicious noodle soup. Restores health and stamina.'
        }
      },
  ...
  if (!inventory.mouseListenersAttached) {
    ...
    document.addEventListener('mousemove', (event) => {
      if (inventory.mouseInteraction.isDragging && inventory.previewObject) {
        ...
        inventory.previewObject.rotation.y +=
          deltaMove.x * inventory.mouseInteraction.rotationSpeed
        inventory.previewObject.rotation.x +=
          deltaMove.y * inventory.mouseInteraction.rotationSpeed
        ...
        inventory.previewRenderer.render(
          inventory.previewScene,
          inventory.previewCamera,
        )
      }
    })
    ...
    document.addEventListener(
      ...
      (event) => {
        if (
          inventory.mouseInteraction.isDragging &&
          inventory.previewObject &&
          event.touches.length === 1
        ) {
          const deltaMove = {
            x:
              event.touches[0].clientX -
              inventory.mouseInteraction.previousMousePosition.x,
            y:
              event.touches[0].clientY -
              inventory.mouseInteraction.previousMousePosition.y,
          }
...
export { initializeInventory, toggleInventory, addItem, selectItem }
