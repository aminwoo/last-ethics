...
function createPlayer() {
  ...
  const classColor = getPlayerClassColor()
  const classColorObj = new THREE.Color(classColor)
  const darkClassColor = classColorObj.clone().multiplyScalar(0.5)
  const brightClassColor = classColorObj.clone().multiplyScalar(1.3)

  // === BODY CORE ===
  // Modern armored torso with tech aesthetic
  const bodyGeometry = new THREE.CylinderGeometry(0.42, 0.52, 1.2, 12)
  ...
  // Armor plate overlay - colored accent
  const armorPlateGeo = new THREE.BoxGeometry(0.5, 0.6, 0.35)
  ...
  armorPlate.position.set(0, 0.1, 0.15)
  playerModel.add(armorPlate)

  // Tech line accents on armor
  const techLineGeo = new THREE.BoxGeometry(0.52, 0.02, 0.36)
  const techLineMat = new THREE.MeshBasicMaterial({
    color: brightClassColor,
    transparent: true,
    opacity: 0.7,
  })
  for (let i = 0; i < 3; i++) {
    const techLine = new THREE.Mesh(techLineGeo, techLineMat)
    techLine.position.set(0, -0.1 + i * 0.15, 0.15)
    playerModel.add(techLine)
  }

  // Belt with pouches
  const beltGeometry = new THREE.TorusGeometry(0.48, 0.05, 8, 20)
  ...
  // Belt buckle - glowing accent
  const buckleGeo = new THREE.BoxGeometry(0.15, 0.1, 0.08)
  const buckleMat = new THREE.MeshBasicMaterial({
    color: classColor,
  })
  const buckle = new THREE.Mesh(buckleGeo, buckleMat)
  buckle.position.set(0, -0.5, 0.5)
  playerModel.add(buckle)

  // === SHOULDER PADS ===
  const shoulderGeometry = new THREE.SphereGeometry(0.2, 10, 10)
  ...
  leftShoulder.position.set(-0.52, 0.4, 0)
  leftShoulder.scale.set(1.1, 0.7, 1.1)
  playerModel.add(leftShoulder)

  // Shoulder accent glow
  const shoulderAccentGeo = new THREE.RingGeometry(0.12, 0.16, 8)
  const shoulderAccentMat = new THREE.MeshBasicMaterial({
    color: classColor,
    side: THREE.DoubleSide,
    transparent: true,
    opacity: 0.8,
  })
  const leftShoulderAccent = new THREE.Mesh(
    shoulderAccentGeo,
    shoulderAccentMat,
  )
  leftShoulderAccent.position.set(-0.58, 0.4, 0)
  leftShoulderAccent.rotation.y = Math.PI / 2
  playerModel.add(leftShoulderAccent)

  const rightShoulder = new THREE.Mesh(shoulderGeometry, shoulderMaterial)
  rightShoulder.position.set(0.52, 0.4, 0)
  rightShoulder.scale.set(1.1, 0.7, 1.1)
  playerModel.add(rightShoulder)

  const rightShoulderAccent = new THREE.Mesh(
    shoulderAccentGeo,
    shoulderAccentMat,
  )
  rightShoulderAccent.position.set(0.58, 0.4, 0)
  rightShoulderAccent.rotation.y = -Math.PI / 2
  playerModel.add(rightShoulderAccent)

  // === HEAD & HELMET ===
  const headGeometry = new THREE.SphereGeometry(0.26, 14, 14)
  ...
  // Helmet ridge accent
  const ridgeGeo = new THREE.TorusGeometry(0.28, 0.02, 6, 16, Math.PI)
  const ridgeMat = new THREE.MeshBasicMaterial({ color: classColor })
  ...
  // Full-face visor - glowing
  const visorGeometry = new THREE.BoxGeometry(0.38, 0.12, 0.08)
  ...
  // Visor frame
  const visorFrameGeo = new THREE.BoxGeometry(0.42, 0.16, 0.04)
  ...
  visorFrame.position.set(0, 0, 0.22)
  head.add(visorFrame)

  // Create arms with better proportions
  ...
  // Create legs - slightly darker shade of class color
  const legColor = new THREE.Color(classColor).multiplyScalar(0.8)
...
function animatePlayerLegs(player, isMoving, deltaTime, isShiftPressed) {
  ...
  if (isMoving) {
    ...
    userData.animationTime +=
      deltaTime * userData.walkSpeed * (isShiftPressed ? 1.8 : 1.0)
