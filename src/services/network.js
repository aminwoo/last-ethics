  ...
  if (showDetailedDebug) {
    ...
    if (playerId && window.gameScene) {
      ...
      if (localPlayer) {
        ...
        html +=
          `Position: x=${localPlayer.position.x.toFixed(1)}, ` +
    ...
    if (remotePlayers.size > 0) {
      ...
      remotePlayers.forEach((playerObj, id) => {
        html += `Player ${id}:<br>`
        html +=
          `- Position: x=${playerObj.position.x.toFixed(1)}, ` +
 ...
 */
function updateRemotePlayerFlashlight(
  flashlight,
  playerPosition,
  playerRotation,
) {
  ...
  // Update flashlight position relative to player
      ...
      socket.onopen = () => {
        ...
        setTimeout(() => {
          ...
          if (playerObject) {
            ...
            console.warn(
              'Could not find player object for initial position update',
            )
          }
        }, 500)
      }
  ...
  try {
    ...
    switch (message.type) {
      case 'init':
        ...
        console.log(
          `Server reports ${message.players.length} other players already connected`,
        )
        ...
        if (localPlayer && window.gameState && window.gameState.playerName) {
          ...
          localPlayer.traverse((child) => {
            if (
              ...
              child.element.classList.contains('player-nametag')
            ) {
          ...
          // Add new name tag
          const nameTag = createPlayerNameTag(
            playerId,
        ...
        if (message.players && message.players.length > 0) {
          console.log(
            'Creating remote players from init message:',
            message.players,
          )
        ...
        addChatMessage(
          'system',
          'System',
          'Welcome to the game! Press T to chat.',
          Date.now(),
          true,
        )
        break
      ...
      case 'playerNameUpdate':
        ...
        console.log(
          `Player ${message.playerId} updated name to: ${message.name}`,
        )
      ...
      case 'playerJoined':
        ...
        console.log(
          `Player ${message.player.id} joined at position:`,
...
function addRemotePlayer(playerData, scene) {
  if (remotePlayers.has(playerData.id)) {
    console.log(
      `Remote player ${playerData.id} already exists, not adding again`,
    )
    return // Player already exists
  }

  console.log(
    `Creating remote player ${playerData.id} at position:`,
    playerData.position,
  )
  ...
  console.log(
    `Remote player ${playerData.id} added to scene at position:`,
...
function removeRemotePlayer(playerId, scene) {
  ...
  console.log(
    `Remote player ${playerId} removed from scene (cleaned up ${css2DObjects.length} name tags)`,
  )
...
function updatePlayerNameTag(playerObject, newName) {
  ...
  playerObject.traverse((child) => {
    if (child instanceof CSS2DObject) {
      ...
      if (
        nameTagElement &&
        nameTagElement.classList.contains('player-nametag')
      ) {
...
function updateRemotePlayer(playerData) {
  ...
  if (remotePlayer) {
    ...
    console.warn(
      `Received update for unknown player ${playerData.id}, adding them now`,
    )
...
function calculateRemotePlayerGunPosition(playerObject, weaponType) {
  ...
  playerObject.traverse((child) => {
    ...
    if (child.name) {
      ...
      if (
        (weaponType === 'Pistol' && lowerName.includes('pistol')) ||
      ...
      if (lowerName.includes('barrel')) {
        ...
        if (
          ...
          direction
            .set(0, 0, 1)
            .applyQuaternion(child.getWorldQuaternion(new THREE.Quaternion()))
  ...
  if (weaponObj && gunTip.length() === 0) {
    ...
    if (barrel) {
      ...
      direction
        .set(0, 0, 1)
        .applyQuaternion(weaponObj.getWorldQuaternion(new THREE.Quaternion()))
    }
  }
...
function createRemotePlayerMuzzleFlash(playerObject, weaponType) {
  ...
  const { gunTip, direction } = calculateRemotePlayerGunPosition(
    playerObject,
    weaponType,
  )

  // Create visual muzzle flash (no PointLight for performance)
  ...
  const flashGeo = new THREE.SphereGeometry(flashSize, 6, 6)
  ...
  flashMesh.position.copy(gunTip)
  scene.add(flashMesh)
...
function createRemotePlayerBullets(scene, playerObject, weaponType) {
  ...
  for (let i = 0; i < bulletsPerShot; i++) {
    ...
    // Add bullet to scene (no PointLight for performance)
    scene.add(bullet)

    // Calculate velocity
    const velocity = bulletDirection
      .clone()
      .multiplyScalar(
        weaponType === 'Shotgun'
...
function createPlayerDeathEffect(position, scene) {
  ...
  // Visual flash at death position (no PointLight for performance)
