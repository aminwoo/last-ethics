...
function updateUI(ui, gameState) {
  ...
  if (ui.classDisplay && gameState.playerClass) {
    const classData = getClass(gameState.playerClass)
    ui.classIcon.textContent = classData.icon
    ui.classNameDisplay.textContent = classData.name
    ui.classDisplay.className = gameState.playerClass
  }
  ...
  if (healthPercent < 25) {
    ui.healthBar.style.backgroundImage =
      'linear-gradient(90deg, rgba(255,0,0,1) 0%, rgba(255,50,0,1) 50%, rgba(255,0,0,1) 100%)'
    ui.healthBar.classList.add('low-health')
  } else if (healthPercent < 50) {
    ui.healthBar.style.backgroundImage =
      'linear-gradient(90deg, rgba(255,100,0,1) 0%, rgba(255,150,0,1) 50%, rgba(255,100,0,1) 100%)'
    ui.healthBar.classList.remove('low-health')
  } else {
    ui.healthBar.style.backgroundImage =
      'linear-gradient(90deg, rgba(255,30,30,1) 0%, rgba(255,70,70,1) 50%, rgba(255,30,30,1) 100%)'
    ui.healthBar.classList.remove('low-health')
  }
  ...
  // Update weapon image - always log for debugging
  console.log('updateUI weapon check:', {
    hasWeaponImage: !!ui.weaponImage,
    hasWeapon: !!gameState.weapon,
    weaponName: gameState.weapon?.name,
    hasImage: !!gameState.weapon?.image,
    currentIndex: gameState.currentWeaponIndex,
  })
  if (ui.weaponImage && gameState.weapon && gameState.weapon.image) {
  ...
  if (obstacles && obstacles.length > 0) {
    ...
    obstacles.forEach((obstacle) => {
      ...
      if (Math.abs(relX) < size / 2 && Math.abs(relZ) < size / 2) {
        ...
        ctx.fillRect(relX - width / 2, relZ - height / 2, width, height)
      }
    })
  }
  ...
  if (zombies && zombies.length > 0) {
    zombies.forEach((zombie) => {
      ...
      if (Math.abs(relX) < size / 2 && Math.abs(relZ) < size / 2) {
        ...
        if (
          ui.minimap.pulseActive &&
          distanceFromCenter <= ui.minimap.pulseRadius
        ) {
        ...
        if (ui.minimap.zombieVisibility[zombieId]) {
          ...
          const timeSinceReveal =
            currentTime - ui.minimap.zombieVisibility[zombieId].lastRevealTime
