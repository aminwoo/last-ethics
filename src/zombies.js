...
export function createZombie(scene, position, type = 'REGULAR', playerRef) {
  ...
  const decayMaterial = new THREE.MeshStandardMaterial({
    color: new THREE.Color(visuals.skinColor).multiplyScalar(0.5),
    roughness: 0.95,
    metalness: 0.0,
  })

  const clothMaterial = new THREE.MeshStandardMaterial({
    color: visuals.clothColor,
    roughness: 0.9,
    metalness: 0.0,
  ...
  const torsoGeometry = new THREE.CapsuleGeometry(
    zombieType.size.width * 0.38,
    zombieType.size.height * 0.28,
    6,
    12,
  )
  const torso = new THREE.Mesh(torsoGeometry, skinMaterial)
  torso.position.y = zombieType.size.height * 0.55
  torso.name = 'torso'
  torso.castShadow = true
  zombie.add(torso)

  // Torn shirt/vest remnants
  const vestGeo = new THREE.BoxGeometry(
    zombieType.size.width * 0.75,
    zombieType.size.height * 0.35,
    zombieType.size.depth * 0.6,
  )
  const vest = new THREE.Mesh(vestGeo, clothMaterial)
  vest.position.y = 0
  vest.scale.set(1, 1, 0.8)
  torso.add(vest)

  // Decay patches on torso
  if (visuals.decay > 0.6) {
    const patchGeo = new THREE.CircleGeometry(zombieType.size.width * 0.15, 6)
    ...
    for (let i = 0; i < 3; i++) {
      ...
      patch.position.set(
        (Math.random() - 0.5) * zombieType.size.width * 0.4,
        (Math.random() - 0.5) * zombieType.size.height * 0.2,
        zombieType.size.depth * 0.3 + 0.01,
      )
  ...
  const headGeometry = new THREE.SphereGeometry(
    zombieType.size.width * 0.32,
    10,
    10,
  )
  const headMaterial = new THREE.MeshStandardMaterial({
    color: new THREE.Color(visuals.skinColor).offsetHSL(0, -0.05, -0.1),
    roughness: 0.85,
    metalness: 0.0,
  })
  ...
  // Sunken eye sockets for creepy look
  const socketGeo = new THREE.SphereGeometry(zombieType.size.width * 0.1, 8, 8)
  ...
  leftSocket.position.set(
    -zombieType.size.width * 0.12,
    0.02,
    zombieType.size.width * 0.22,
  )
  leftSocket.scale.set(1, 0.8, 0.5)
  head.add(leftSocket)

  const rightSocket = new THREE.Mesh(socketGeo, socketMat)
  rightSocket.position.set(
    zombieType.size.width * 0.12,
    0.02,
    zombieType.size.width * 0.22,
  )
  rightSocket.scale.set(1, 0.8, 0.5)
  head.add(rightSocket)

  // Glowing eyes - piercing and menacing
  const eyeGeometry = new THREE.SphereGeometry(
    zombieType.size.width * 0.055,
    8,
    8,
  )
  const eyeMaterial = new THREE.MeshBasicMaterial({
    color: visuals.eyeColor,
  })
  ...
  leftArm.position.set(
    -zombieType.size.width * 0.55,
    zombieType.size.height * 0.58,
    0,
  )
  ...
  // Cache body parts in userData for fast access (avoids traverse() every frame)
  zombie.userData.bodyParts = {
    head: head,
    torso: torso,
    leftArm: leftArm,
    rightArm: rightArm,
...
export function updateZombies(deltaTime) {
  ...
  for (let i = zombies.length - 1; i >= 0; i--) {
    ...
    if (timeSinceHit < 200) {
      // Flash red when hit - use cached body parts instead of traverse()
      const bodyParts = userData.bodyParts
      if (bodyParts) {
        const partsToFlash = [
          bodyParts.head,
          bodyParts.torso,
        ...
        const emissiveIntensity = 1 - timeSinceHit / 200

        for (const child of partsToFlash) {
          if (child && child.isMesh && child.material) {
            // Check if this material type supports emissive properties
            if (child.material.emissive) {
              // Clone material once to avoid affecting other instances
      ...
      // Add stagger effect when knocked back - use cached body parts
      if (
        userData.knockback &&
        userData.knockback.active &&
        userData.bodyParts
      ) {
        const { head, torso, leftLeg, rightLeg, leftArm, rightArm } =
          userData.bodyParts
        ...
        if (leftArm && rightArm) {
          leftArm.rotation.x =
            -Math.PI * 0.1 + Math.sin(currentTime * 0.02) * staggerFactor
          rightArm.rotation.x =
            -Math.PI * 0.1 - Math.sin(currentTime * 0.02) * staggerFactor
      ...
      // Reset materials after flash - use cached body parts
      const bodyParts = userData.bodyParts
      if (bodyParts) {
        const partsToReset = [
          bodyParts.head,
        ...
        for (const child of partsToReset) {
          if (child && child.isMesh && child.material) {
            ...
            // Restore original material if we stored it
            if (child.userData.originalMaterial) {
      ...
      // Reset stagger animations if knockback has ended - use cached body parts
      if (
        userData.knockback &&
        !userData.knockback.active &&
        userData.bodyParts
...
// Animate zombie idle state (subtle swaying) - uses cached body parts
function animateZombieIdle(zombie, time) {
  const bodyParts = zombie.userData.bodyParts
  if (!bodyParts) return

  const { head, leftArm, rightArm, torso } = bodyParts
...
// Animate zombie walking - uses cached body parts
function animateZombieWalking(zombie, time) {
  const bodyParts = zombie.userData.bodyParts
  if (!bodyParts) return

  const { head, leftArm, rightArm, leftLeg, rightLeg, torso } = bodyParts
...
// Animate zombie attacking - uses cached body parts
...
// Animate zombie dying - uses cached body parts
function animateZombieDying(zombie, progress) {
  ...
  if (progress <= 0.3) {
    ...
    if (leftArm) {
      leftArm.rotation.z =
        1.0 + Math.sin(groundProgress * 10) * 0.1 * (1 - groundProgress)
    }

    if (rightLeg) {
      rightLeg.rotation.z =
        Math.sin(groundProgress * 8) * 0.15 * (1 - groundProgress)
    }
  }
}
