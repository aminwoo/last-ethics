...
export const weapons = [
  ...
  },
  {
    ...
    isReloading: false,
    reloadStartTime: 0,
    image: '/images/[design]%20Assault_rifle_V1.00.png',
    type: 'ranged',
    isAutomatic: true,
    bulletsPerShot: 1,
    color: 0xff0000,
...
// Reusable Vector3 objects for bullet collision detection
...
export function handleWeaponSwitch(
  ...
  if (switchWeaponFn(gameState, weaponIndex)) {
    ...
    if (player && player.userData && player.userData.weapons) {
      const weapons = player.userData.weapons
      // Hide all weapons first
      if (weapons.pistol) weapons.pistol.visible = false
      if (weapons.shotgun) weapons.shotgun.visible = false
      if (weapons.assaultRifle) weapons.assaultRifle.visible = false
      if (weapons.sniperRifle) weapons.sniperRifle.visible = false
      if (weapons.bat) weapons.bat.visible = false

      // Show the selected weapon based on index
      switch (weaponIndex) {
        case 0:
          if (weapons.pistol) weapons.pistol.visible = true
          break
        case 1:
          if (weapons.shotgun) weapons.shotgun.visible = true
          break
        case 2:
          if (weapons.assaultRifle) weapons.assaultRifle.visible = true
          break
        case 3:
          if (weapons.sniperRifle) weapons.sniperRifle.visible = true
          break
        case 4:
          if (weapons.bat) weapons.bat.visible = true
          break
      }
    }
...
export function createMuzzleFlash(player, scene) {
  ...
  if (userData.weapons.shotgun && userData.weapons.shotgun.visible) {
    ...
    userData.weapons.assaultRifle &&
    userData.weapons.assaultRifle.visible
  ) {
    currentWeapon = 'rifleBarrel'
  } else if (
    userData.weapons.sniperRifle &&
    userData.weapons.sniperRifle.visible
  ) {
    currentWeapon = 'sniperBarrel'
  }

  const barrelName =
    currentWeapon === 'rifleBarrel' || currentWeapon === 'sniperBarrel'
      ? currentWeapon
  ...
  if (gunBarrel) {
    ...
    if (currentWeapon === 'shotgun') {
      for (let i = 0; i < 5; i++) {
        ...
        const smallFlash = new THREE.Mesh(
          smallFlashGeometry,
          smallFlashMaterial,
        )
    ...
    if (currentWeapon === 'sniperBarrel') {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        const trailFlash = new THREE.Mesh(
          trailFlashGeometry,
          trailFlashMaterial,
        )
    ...
    try {
      ...
      // Create a small sphere (no PointLight for performance)
...
export function handleShooting(input, player, scene, gameState) {
  ...
  if (weapon.ammo > 0) {
    // Check if ammo should be consumed (engineer class has chance to save ammo)
    const classStats = gameState.classStats || {}
    if (shouldConsumeAmmo(classStats)) {
      weapon.ammo--
    }
...
function createBulletImpact(position, scene, surface = 'ground') {
  // Create a particle effect for bullet impact (reduced count for performance)
  const particleCount = surface === 'zombie' ? 12 : surface === 'ground' ? 8 : 5
  const particleGeometry = new THREE.SphereGeometry(0.03, 4, 4)

  // Different colors based on impact surface
  let particleColor, secondaryColor
  if (surface === 'zombie') {
    particleColor = 0x8b0000 // Dark red for blood
    secondaryColor = 0xff0000 // Bright red for variation
  ...
  // Create particles with varied materials (no PointLight for performance)
  for (let i = 0; i < particleCount; i++) {
    const color = Math.random() > 0.5 ? particleColor : secondaryColor
    ...
    // Adjust particle size for blood - more variation
    if (surface === 'zombie') {
      const scale = 0.3 + Math.random() * 1.5 // More varied sizes for blood droplets
    ...
    const speed =
      surface === 'zombie' ? 2.5 + Math.random() * 6 : 1 + Math.random() * 3
  ...
  const animateParticles = () => {
    ...
    for (let i = particles.length - 1; i >= 0; i--) {
      ...
      if (p.surface === 'zombie') {
        p.direction.y -= 0.06 // Slightly stronger gravity effect for blood
        p.speed *= 0.97 // Blood slows down faster than other particles
      }

      // Decrease life and opacity
      p.life -= p.surface === 'zombie' ? 0.018 : 0.01 // Slightly slower fade for blood
...
export function switchWeapon(gameState, weaponIndex) {
  if (
    ...
    weaponIndex !== gameState.currentWeaponIndex
  ) {
...
export function checkReloadCompletion(gameState) {
  ...
  // Calculate reload time with class modifier
  const classStats = gameState.classStats || {}
  const effectiveReloadTime = getReloadTime(weapon.reloadTime, classStats)

  // Check if reload time has passed
  if (elapsedTime >= effectiveReloadTime) {
...
function shootBullet(input, weapon, player, scene) {
  ...
  if (weapon.name === 'Pistol' && player.userData.weapons.pistol) {
    ...
    player.userData.weapons.assaultRifle
  ) {
    weaponObj = player.userData.weapons.assaultRifle
  } else if (
    weapon.name === 'Sniper Rifle' &&
    player.userData.weapons.sniperRifle
  ) {
  ...
  if (barrel) {
    ...
    gunDirection
      .set(0, 0, 1)
      .applyQuaternion(barrel.getWorldQuaternion(new THREE.Quaternion()))
  } else {
  ...
  for (let i = 0; i < bulletsPerShot; i++) {
    ...
    if (weapon.name === 'Shotgun' && bulletsPerShot > 1) {
      ...
      const perpX = new THREE.Vector3()
        .crossVectors(initialDirection, new THREE.Vector3(0, 1, 0))
        .normalize()
      const perpY = new THREE.Vector3()
        .crossVectors(initialDirection, perpX)
        .normalize()
    ...
    // Add bullet to scene and tracking array (no PointLight for performance)
    scene.add(bullet)
    ...
    // Calculate damage with class modifiers (including critical hit chance)
    const classStats = window.gameState?.classStats || {}
    const finalDamage = calculateDamage(weapon.damage, classStats)
...
export function updateBullets(scene, zombies = []) {
  ...
  for (let i = bullets.length - 1; i >= 0; i--) {
    ...
    if (zombies && zombies.length > 0) {
      _bulletPos.copy(bullet.mesh.position)
      ...
      for (let j = 0; j < zombies.length; j++) {
        ...
        if (
          ...
          (bullet.piercedZombies && bullet.piercedZombies.has(zombie.id))
        ) {
          continue
        }
        ...
        if (distance < bulletRadius + zombieRadius) {
          ...
          _impactPos.copy(_bulletPos)
          _toZombie.subVectors(_zombiePos, _bulletPos).normalize()
          _impactPos.addScaledVector(_toZombie, bulletRadius)

          // Create blood impact effect
          createBulletImpact(_impactPos, scene, 'zombie')
          ...
          const knockbackAmount =
            bullet.damage * (bullet.weaponType === 'Shotgun' ? 0.1 : 0.5)
          ...
          if (typeof zombie.takeDamage === 'function') {
            ...
            zombie.userData &&
            typeof zombie.userData.takeDamage === 'function'
          ) {
            // Fall back to userData.takeDamage
            zombie.userData.takeDamage(
              bullet.damage,
              bullet.direction,
              knockbackAmount,
            )
            ...
            zombie.userData &&
            typeof zombie.userData.onHit === 'function'
          ) {
            ...
            if (zombie.userData.knockback) {
              const knockbackForce = bullet.direction
                .clone()
                .multiplyScalar(
                  bullet.damage *
                    0.02 *
                    (bullet.weaponType === 'Shotgun' ? 0.1 : 0.5),
                )
          ...
          if (damageApplied) {
            console.log(
              `Zombie hit! Damage: ${bullet.damage}, Knockback: ${knockbackAmount.toFixed(2)}`,
            )
          }
    ...
    const shouldRemovePiercingBullet =
      bullet.canPierce && bullet.pierceCount >= bullet.maxPierceCount
...
function createPiercingEffect(bullet, zombie, scene) {
  ...
  particles.userData = {
    velocities: Array(particleCount)
      .fill()
      .map(() => {
        ...
        return new THREE.Vector3(
          ...
          .normalize()
          .multiplyScalar(Math.random() * 0.2 + 0.05)
      }),
    createdAt: Date.now(),
  }
  scene.add(particles)

  // Animate the effect (no PointLight for performance)
  const animateEffect = () => {
